...
class CartService {
  static Future<Map<String, dynamic>> getCart() async {
    ...
    try {
      var url = Uri.https(Config.apiLocalhost, '${Config.getCart}/get');
  ...
  static Future<void> addToCart(int productId, int quantity) async {
    ...
    var client = http.Client();
    try {
      var url = Uri.https(Config.apiLocalhost, '${Config.getCart}/add');
  ...
  static Future<void> removeFromCart(int id) async {
    ...
    var client = http.Client();
    try {
      var url = Uri.https(Config.apiLocalhost, '${Config.getCart}/remove/$id');
  ...
  static Future<void> increaseQuantity(int id) async {
    ...
    try {
      var url =
          Uri.https(Config.apiLocalhost, '${Config.getCart}/increase/$id');
  ...
  static Future<void> decreaseQuantity(int id) async {
    ...
    try {
      var url =
          Uri.https(Config.apiLocalhost, '${Config.getCart}/decrease/$id');
      var response = await client.patch(
        url,
        headers: {'Authorization': 'Bearer $token'},
      );
  ...
  static Future<void> clearCart() async {
    ...
    var client = http.Client();
    try {
      var url = Uri.https(Config.apiLocalhost, '${Config.getCart}/clear');
      var response = await client.delete(
        url,
        headers: {'Authorization': 'Bearer $token'},
      );
  ...
  static Future<Products> getProductDetails(int productId) async {
    var client = http.Client();
    var url = Uri.https(Config.apiLocalhost, '/api/v1/product/get/$productId');
